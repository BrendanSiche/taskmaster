...
class Config():
    ...
    def check_config(config):
        for elem in config['programs']:
            ...
            if config['programs'][elem].get('autostart') != None and type(config['programs'][elem].get('autostart')) != bool:
                log_and_print(elem + " The auttostart parameter must be a bool (true or false)")
            if config['programs'][elem].get('autorestart') != None and (type(config['programs'][elem].get('autorestart')) != str and type(config['programs'][elem].get('autorestart')) != bool):
