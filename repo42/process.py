...
def check_validfile(switch = None):
    ...
    if len(sys.argv) == 2:
        ...
        config.Config.creat_defconfig(switch)
        arg = 'defconf.yaml'
    with open(arg, 'r') as yaml_file:
        try:
            param = yaml.safe_load(yaml_file)
        except:
            print(f"{tskconsol.Tcolors.CRO}", tskconsol.Tcolors.colorize(tskconsol.Tcolors.UDRL + " Ivalid Config File : badly indented Yaml file \n",91))
            exit()
...
def how_many_running(param):
    ...
    for elem in running[name]:
        if elem['process'].poll() != None:
            ...
            if check_uptime(elem['start_time'], param.get('starttime')) == True: 
                elem['confirm'] = 'True'
...
def check_on_process(param):
    ...
    for elem in ran:
        ...
        if elem['process'].poll() not in param.get('exitcodes') and param.get('manstop') != True:
            ...
            if param.get('autorestart') == 'unexpected' and param.get('manstop') != True:
...
def follow_conf_launch(param):
    ...
    if j > 0:
        ...
        for x in range(j):
            if running.get(name) != None:
                running[name][x]['process'].kill()
                logging.info(f"Forcefully killed process  : {param['cmd']} of pid: {str(running[name][x]['process'].pid)}")
                print('Forcefully killed process ', param['cmd'], 'of pid: ', str(running[name][x]['process'].pid))
    time.sleep(0.1)
    ran, done, cur = how_many_running(param)
    name = param['cmd']
    logging.info(f"Exec Process : {name}")
    logging.info(f"Numprocs : {param['numprocs']}")
